...
<?php }?>
				...
				lihtml+='<br>菜单URL：<?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=<span class="nowmodaction">'+$('#folder').val()+'</span>&file=<span class="nowmodaction">'+$('#folder').val()+'</span>&action= <input type="text" class="common-text" name="menu[action]['+ct+']" placeholder="菜单action" size="16" /></li>';
...
<body>
    <div class="ifame-main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?file=index&action=main">管理首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">自定义模块</span></div>
        </div>
                        ...
                        <table width="100%" class="insert-tab">
                            <tbody>
                            	<tr class="formtr">
                                    ...
                                    <td class="formtd">
                                 	<select name="info[parentkey]">
                                        <?php $no=1;if(is_array(Module::moduleList(0)))foreach(Module::moduleList(0) as $r){?>
                                        <?php if($r['folder']) { ?>
                                        <option value="<?php echo isset($r['key'])?$r['key']:'';?>"><?php echo isset($r['name'])?$r['name']:'';?></option>
                                        <?php }?>
                                        <?php $no++;}?>
                                    </select>
									<br>
									<label style="color:#FF3300"><input type="checkbox" class="common-checkbox" value="1" name="info[ispch5]">这是一个兼具PC+公众号的模块（如果选中此项，程序将自动调用2套视图和2个控制器进行Pc和微信端的切换）</label>
                                    </td>
                                </tr>
                            	<tr class="formtr">
                                ...
                                <tr class="formtr">
                                    ...
                                    <br>
                                    菜单URL：<?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=<span class="nowmodaction"></span>&file=<span class="nowmodaction"></span>&action= <input type="text" class="common-text" name="menu[action][0]" value="config" size="16" />  
                                    &nbsp;&nbsp;
                                    <a href="javascript:void(0);" onClick="appendModuleElement();">增加菜单项</a>
                                    </li>
                                    </ul>
                                    <a href="javascript:void(0);" onMouseOver="$('#module_diy_demo_img').show();" onMouseOut="$('#module_diy_demo_img').hide();" style="position:relative">参数说明<img id="module_diy_demo_img" src="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>admin/template/images/module_diy_demo.jpg" style="position:absolute; display:none; top:24px; left:0px"></a>
                                    </td>
                                </tr>
								<tr class="formtr">
                                    <th class="formth"><i class="require-red">*</i>是否含有会员中心：</th>
                                    <td class="formtd">
										<label><input type="radio" checked="checked" name="info[isuc]" class="common-radio" value="0"> 无会员中心</label>
										&nbsp;&nbsp;
										<label><input type="radio" name="info[isuc]" class="common-radio" value="1"> 仅含微信端会员中心</label>
										&nbsp;&nbsp;
										<label><input type="radio" name="info[isuc]" class="common-radio" value="2"> 仅含PC端会员中心</label>
										&nbsp;&nbsp;
										<label><input type="radio" name="info[isuc]" class="common-radio" value="3"> 含微信+PC端会员中心</label>
									</td>
								</tr>
                                <tr class="formtr">
