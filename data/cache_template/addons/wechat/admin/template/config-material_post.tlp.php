...
<head>
	...
	<style type="text/css">
		...
		.wechat-editor .editor-right ul li a{display:block; border:#e7e7eb 1px solid; font-size:15px; padding-left: 50px;line-height: 40px; color:#333333; background:url(<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>addons/wechat/admin/template/images/appmsg_new_z2ac3d8.png) 20px 12px no-repeat}
...
<body>
    <div class="ifame-main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?file=index&action=main">管理首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">新建图文素材</span></div>
        </div>
        <div class="wechat-editor">
			...
			<div class="editor-left">
				...
				<div class="all-tuwen">
					<div class="big-tuwen">
						<input type="hidden" name="Title[0]" id="title_0" />
						<input type="hidden" name="Author[0]" id="author_0" />
						<input type="hidden" name="Description[0]" id="description_0" />
						<input type="hidden" name="PicUrl[0]" id="picurl_0" />
						<input type="hidden" name="thumb_media_id[0]" id="thumb_media_id_0" />
						<input type="hidden" name="content[0]" id="content_0" />
						<input type="hidden" name="urls[0]" id="url_0" />
						<img id="img_0" src="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>addons/wechat/admin/template/images/no_cover_pic.png" /><span id="title_span_0">标题</span>
						<div class="big-tuwen-hover">
							<a href="javascript:void(0);" onClick="editMsg(0);">编辑</a>
						</div>
					</div>
					<div class="add-tuwen">
						<a href="javascript:void(0);"><img  src="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>addons/wechat/admin/template/images/tuwen_editor_add.png" /></a>
					</div>
				</div>
			</div>
			<div class="editor-middel">
				<span class="arrow"><img src="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>addons/wechat/admin/template/images/area_arrow.png"></span>
				<table width="100%" class="insert-tab">
					<tbody>
						<tr class="formtr">
							<th class="formth" width="20%"><i class="require-red">*</i>标题：</th>
							<td class="formtd"><input type="text"  style="width:493px;" name="p_title" id="p_title" class="common-text"></td>
						</tr>
						<tr class="formtr">
							<th class="formth">作者：</th>
							<td class="formtd"><input type="text" size="32" name="p_author" value="<?php echo defined('WECHAT_NAME')?WECHAT_NAME:'{__WECHAT_NAME__}';?>" id="p_author" class="common-text"></td>
						</tr>
						<tr class="formtr">
							<th class="formth"><i class="require-red">*</i>封面图片：</th>
							<td class="formtd" style="position:relative">
							<?php echo Form::image('封面图片','p_picurl',PW_PATH.'addons/wechat/admin/template/images/no_cover_pic.png');?>
						...
						<tr class="formtr">
							...
							<td class="formtd">
							<textarea class="common-textarea" style="width:493px; height:120px;" name="p_description" id="p_description"></textarea>
							</td>
						</tr>
						<tr class="formtr">
							<th class="formth"><i class="require-red">*</i>正文：</th>
							<td class="formtd" style="padding:10px">
							<?php echo Form::baiduEditor('正文','p_content');?>
							
							</td>
						</tr>
						<tr class="formtr">
							<th class="formth" width="20%">外链：</th>
							<td class="formtd"><input type="text" style="width:493px;" name="p_url" id="p_url" class="common-text"></td>
						</tr>
						<tr class="formtr">
							<th class="formth"></th>
							<td class="formtd">
	...
	$(document).ready(function(){
		...
		$(".add-tuwen").click(function(){
			...
			var html='<div class="small-tuwen" data-id="'+cur+'" id="small-tuwen-'+cur+'">'+
						...
						'</div>'+
						'<div class="small-img">'+
						'	<img id="img_'+cur+'" src="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>addons/wechat/admin/template/images/no_cover_pic.png" />'+
						'</div>'+
	...
	function editMsg(id)
	{
		...
		{
			$('#img_demop_picurl').attr('src','<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>statics/images/demo.png');
		}
	...
	function moveUp(id)
	{
		...
		if(index==1)
		{
			...
			{
				$('#img_0').attr('src','<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>addons/wechat/admin/template/images/no_cover_pic.png');
			}
			...
			{
				$('#img_'+id).attr('src','<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?>addons/wechat/admin/template/images/no_cover_pic.png');
			}
	...
	<div id="material-dialog-thumb" class="reveal-modal" style="width:880px; height:550px">
		...
		<ul id="image-jquery-data-thumb">
			<?php $no=1;if(is_array($imageJuqeryList))foreach($imageJuqeryList as $r){?>
				<li><a href="javascript:void(0);" onClick="insertImageToThumb('<?php echo isset($r['media_id'])?$r['media_id']:'';?>','<?php echo isset($r['local_url'])?$r['local_url']:'';?>')"><img src="<?php echo format_url($r['local_url']);?>" /></a></li>
			<?php $no++;}?>
		</ul>
	...
	<div id="material-dialog" class="reveal-modal" style="width:880px; height:550px">
		...
		<ul id="image-jquery-data">
			<?php $no=1;if(is_array($imageJuqeryList))foreach($imageJuqeryList as $r){?>
				<li><a href="javascript:void(0);" onClick="insertImageToContent('<?php echo isset($r['remote_url'])?$r['remote_url']:'';?>');"><img src="<?php echo format_url($r['local_url']);?>" /></a></li>
			<?php $no++;}?>
		</ul>
		<a class="close-reveal-modal">&#215;</a>
	</div>
