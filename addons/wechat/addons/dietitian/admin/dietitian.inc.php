...
$mod=@return_edefualt(str_callback_w($_GET['mod']),'dietitian');
$file=@return_edefualt(str_callback_w($_GET['file']),'dietitian');
...
switch($action)
{
	...
	case 'config':
		if(isset($dosubmit))
		{
			...
			operation_tips('操作成功！','?mod=dietitian&file=dietitian&action=config');
		}
	...
	//case 'manage':
		...
		//在此写 phpwechat.php?mod=dietitian&file=dietitian&action=manage 时的逻辑
			
		//break;

	//case 'add':
			
		//在此写 phpwechat.php?mod=dietitian&file=dietitian&action=add 时的逻辑
	...
	//case '...':
		...
		//在此写 phpwechat.php?mod=dietitian&file=dietitian&action=... 时的逻辑
